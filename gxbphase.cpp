...
#include <time.H>
#include <fstream>
#include <Lisense\dogLisense.h>
#pragma comment(lib,"G:\\opencv3.1\\x64\\vc12\\lib\\opencv_world310.lib")
/*==========================
变量声明
===========================*/
	...
	for (int i = 0; i < H; i++)
	{
		...
		for (int l = 0; l < W; l++)
		{
			...
			if (abPhase[l] >= abPhase22[l])
			{
				...
				ababPhase[l] = 2 * PI*(int)((double(p2*p3/(p3-p2)) / (p2*p3/(p3 - p2)-p1*p2/(p2-p1)) * delta - abPhase[l]) / (2 * PI)) + abPhase[l];
